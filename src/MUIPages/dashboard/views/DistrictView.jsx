...
export default function DistrictView({ setAppendMessage }) {
  ...
  const renderExtraDataTable = () => {
    if (selectedPolygon && extraData[selectedPolygon.properties.NAME_3.toLowerCase()]) {
      const districtData = extraData[selectedPolygon.properties.NAME_3.toLowerCase()];
      return (
        <Paper sx={{ p: 2, mb: 2 }}>
              ...
              <TableBody>
                {Object.entries(districtData).map(([key, value]) => (
                  <React.Fragment key={key}>
                    <TableRow>
                      <TableCell colSpan={2} sx={{ fontWeight: 'bold' }}>{key}</TableCell>
                    </TableRow>
    ...
    <Box sx={{ display: 'flex', height: isMobile ? 'auto' : '90vh', width: "100%", p: 2, flexDirection: isMobile ? 'column' : 'row' }}>
      ...
      <Box sx={{ width: isMobile ? "100%": '50%', height: '100%', overflowY: 'auto', pl: 2, pt: 1 }}>
        <FormControl sx={{ width: "100%", mb: 2 }}>
          <InputLabel id="polygon-select-label">Select District</InputLabel>
          <Select
            labelId="polygon-select-label"
            id="polygon-select"
            value={selectedPolygon ? selectedPolygon.properties.GID_3 : ''}
